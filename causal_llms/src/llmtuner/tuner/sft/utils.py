...
# todo 修改所有的block prefix，怎么定义呢？

def get_block_param(model, block_id, block_prefix):
    return {name: param.detach().to('cpu') for name, param in model.named_parameters() if name.startswith(block_prefix[block_id][0])}

def update_global(model, block_results, part=1.0):
    up_block = []
    for k, v in block_results.items():
        if len(v) != 0:
            up_block.append(k)
    compressed_block = random.sample(up_block, math.ceil(len(up_block)*part))
    print(len(compressed_block), 'blocks is transing.')
    for k, v in block_results.items():
        if k in compressed_block:
