...
from .block_optim import BlockOptimizer, BlockOptimizerRatio, FLBlockOptimizer
from .lora_optim import LoRAOptimizer
import time
...
class Seq2SeqPeftTrainer(PeftTrainer):
    ...
    """
    def __init__(self, finetuning_args: FinetuningArguments, client_bid=None, client_id=None, block_prefix_list=None, **kwargs):
        ...
        # block to clients dict
        self.client_id = client_id
        self.client_bid = client_bid

        assert block_prefix_list is not None, 'block_prefix_list must be pre defined.'
        self.block_prefix_list = block_prefix_list
        self.optimizer = self.create_optimizer()
        if isinstance(self.optimizer, FLBlockOptimizer):
            self.block_his = self.optimizer.block_track
    ...
    def create_optimizer(self, ):
        ...
        if self.finetuning_args.finetuning_type in ["flblock", "cadap", "cadap_lora"]:
            # make it block-wise optimizer
            print(f"Using federated-block-wise optimizer, and {self.finetuning_args.finetuning_type} is running now.")

            self.optimizer = FLBlockOptimizer(base_optimizer=self.optimizer,
                                        named_parameters_list=list(self.model.named_parameters()),
                                        parameters_list=list(self.model.parameters()),
                                        block_prefix_list=self.block_prefix_list,
                                        switch_block_every=self.finetuning_args.switch_block_every,
                                        switch_mode='fixed',
                                        log_fn=self.log,
                                        start_block=self.client_bid,
                                        verbose=self.finetuning_args.badam_verbose,
                                        power_sgd=self.finetuning_args.power_sgd,
                                        )

        elif self.finetuning_args.finetuning_type == 'block':
        ...
        elif self.finetuning_args.finetuning_type in ['lora', 'adap', 'adap_lora']:
        #     # when using LoRA, make sure the update uses float32 # TODO, fix bug
        #     # self.step = self.casted_step
        #     # self.optimizer = LoRAOptimizer(base_optimizer=self.optimizer,
        #     #                                named_parameters_list=list(self.model.named_parameters()),
        #     #                                log_fn=self.log,
        #     #                                verbose=self.finetuning_args.badam_verbose)
        #     pass
        # elif self.finetuning_args.finetuning_type == "adapter":
            pass
...
# class FLSeq2SeqPeftTrainer(Seq2SeqPeftTrainer):
