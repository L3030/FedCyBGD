...
gpt2_block_list = [*[[f"transformer.h.{i}."] for i in range(12)]]
...
class BlockOptimizer(Optimizer):
    ...
    def __init__(
        ...
        """
        assert block_prefix_list == "gpt2", "block_prefix_list should be pre-defined"

        block_prefix_list = gpt2_block_list
        ...
        self.active_modules = ["qa_outputs."]
        # self.active_modules = []
    ...
    def update_trainable_params(self, verbose: Optional[int] = None) -> None:
        ...
        if isinstance(self.current_block_idx, list):
            self.active_param_prefixs = [self.block_prefix_list[idx][0] for idx in
                                         self.current_block_idx] + self.active_modules
        else:
...
class FLBlockOptimizer(Optimizer):
    ...
    def __init__(
        ...
        block_prefix_list = gpt2_block_list
        assert switch_mode in ["descending", "ascending", "random"]
        assert isinstance(block_prefix_list, list)
...
class BlockOptimizerRatio(Optimizer):
    def __init__(self, param_groups,
        ...
        for n, p in named_parameters_list:
            ...
            self.sparse_dict[p]["seed"] = torch.randint(0, 1000,
                                                        (1,)).item()  # seed for each parameter's random index generator
                ...
                if p.grad is not None:
                    ...
                    if len(state) == 0:
                        ...
                        state['exp_avg'] = torch.zeros_like(
                            p.grad._values())  # NOTE: now the exp_avg is a vector instead of matrix, since we only store the states for the non-zero entries
    ...
    def _generate_mask_adjacent(self, param, ratio, offset):
        ...
        mask = torch.sparse_coo_tensor(unraveled_i, torch.ones(num_ones, device=param.device, dtype=param.dtype),
                                       param.shape)
    ...
    def _generate_mask_scatter(self, param, ratio, offset):
        ...
        torch.random.manual_seed(
            self.sparse_dict[param]["seed"])  # NOTE: comment this seems to provide faster convergence.
        ...
        def func(x):
            ...
            for n, p in iterator:
                ...
                if p.requires_grad and p.grad is not None:
                    ...
                    update_ratio = self.sparse_dict[p]["update_ratio"] if "update_ratio" in self.sparse_dict[
                        p] else self.update_ratio
                    ...
                    if len(self.ordered_named_params) == self.param_num:
                        print(
                            "The parameter's update order has been changed. This may indicate bugs in sparse optimizer.")
            return x
